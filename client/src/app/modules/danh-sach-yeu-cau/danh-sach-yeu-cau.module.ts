import {NgModule} from '@angular/core';
import {CommonModule} from '@angular/common';
import {DialogModule} from 'primeng/dialog';
import {ButtonModule} from 'primeng/button';
import {
    CalendarModule,
    CheckboxModule,
    ConfirmDialogModule,
    DropdownModule,
    EditorModule,
    FileUploadModule,
    InputTextareaModule,
    InputTextModule,
    ListboxModule,
    MultiSelectModule, OverlayPanelModule, PaginatorModule, RadioButtonModule, ScrollPanelModule, TabViewModule,
    TooltipModule, TreeModule,
    TriStateCheckboxModule
} from 'primeng/primeng';
import {ToastModule} from 'primeng/toast';
import {TableModule} from 'primeng/table';
import {FormsModule, ReactiveFormsModule} from '@angular/forms';
import {DanhSachYeuCauRoutes} from "./danh-sach-yeu-cau.routes";
import {DanhSachYeuCauListComponent} from "./danh-sach-yeu-cau-list/danh-sach-yeu-cau-list.component";
import {UpdateDanhSachYeuCauComponent} from "./update-danh-sach-yeu-cau/update-danh-sach-yeu-cau.component";
import {DuyetYeuCauComponent} from "./duyet-yeu-cau/duyet-yeu-cau.component";

@NgModule({
    imports: [
        DanhSachYeuCauRoutes,
        CommonModule,
        DialogModule,
        ButtonModule,
        ConfirmDialogModule,
        ToastModule,
        TableModule,
        TooltipModule,
        ReactiveFormsModule,
        CheckboxModule,
        TriStateCheckboxModule,
        FormsModule,
        MultiSelectModule,
        CalendarModule,
        DropdownModule,
        InputTextModule,
        InputTextareaModule,
        EditorModule,
        FileUploadModule,
        ListboxModule,
        DialogModule,
        PaginatorModule,
        OverlayPanelModule,
        TreeModule,
        DropdownModule,
        ScrollPanelModule,
        TabViewModule,
        RadioButtonModule
    ],
    declarations: [
        DanhSachYeuCauListComponent,
        UpdateDanhSachYeuCauComponent,
        DuyetYeuCauComponent
    ]
})
export class DanhSachYeuCauModule {
}
